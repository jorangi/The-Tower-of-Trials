...
public:
  ...
  StatComponentBuilder &SetSTR(uint32_t val) {
    this->str = val;
    return *this;
  }
  StatComponentBuilder &SetDEX(uint32_t val) {
    this->dex = val;
    return *this;
  }
  StatComponentBuilder &SetINT(uint32_t val) {
    this->_int = val;
    return *this;
  }
  StatComponentBuilder &SetWIS(uint32_t val) {
    this->wis = val;
    return *this;
  }
  StatComponentBuilder &SetCHA(uint32_t val) {
    this->cha = val;
    return *this;
  }
  StatComponentBuilder &SetDEF(uint32_t val) {
    this->def = val;
    return *this;
  }
  StatComponentBuilder &SetSPD(uint32_t val) {
    this->spd = val;
    return *this;
  }
...
private:
  ...
  /// @param def 방어력
  /// @param spd 속도
  StatComponent(uint32_t str, uint32_t dex, uint32_t _int, uint32_t wis,
                uint32_t cha, uint32_t def, uint32_t spd)
      : str(str, "STR"), dex(dex, "DEX"), _int(_int, "INT"), wis(wis, "WIS"),
        cha(cha, "CHA"), def(def, "DEF"), spd(spd, "SPD") {}
...
public:
  ...
  uint32_t GetTypeID() const override {
    return GetComponentTypeID<StatComponent>();
  }
  ...
  void Deserialize(const nlohmann::json &j) override {
    int strBase = 0, dexBase = 0, intBase = 0, wisBase = 0, chaBase = 0,
        defBase = 0, spdBase = 0;
