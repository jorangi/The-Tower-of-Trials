...
TTOT::Core::Scenes::TitleScene::TitleScene(TTOT::Core::GameContext &context)
    : SceneBase(context) {}
...
void TTOT::Core::Scenes::TitleScene::HandleInput(const ftxui::Event &event) {
  ...
  if (event == ftxui::Event::Character('1') ||
      (event == ftxui::Event::Return && selectedIndex == 0)) {
    context.eventBus.Publish(TTOT::Core::Events::SceneChangeEvent{4});
    return;
  } else if (event == ftxui::Event::Character('2') ||
             (event == ftxui::Event::Return && selectedIndex == 1)) {
    outputText = "새 게임 선택됨";
    ...
    return;
             ...
             (event == ftxui::Event::Return && selectedIndex == 3)) {
    outputText = "종료 선택됨";
...
ftxui::Element TTOT::Core::Scenes::TitleScene::Render() {
  ...
  return vbox({vbox({text("The Tower of Trials") | bold |
                     color(Color::Yellow) | hcenter}) |
                   border,
               vbox({text("1. 불러오기") | color(Color::Cyan) |
                         (selectedIndex == 0 ? bold : nothing),
                     text("2. 새 게임") | color(Color::Green) |
                         (selectedIndex == 1 ? bold : nothing),
                     text("3. 설정") | color(Color::White) |
                         (selectedIndex == 2 ? bold : nothing),
                     text("4. 종료") | color(Color::Red) |
                         (selectedIndex == 3 ? bold : nothing)}) |
                     hcenter,
                 separator(),
                 vbox({text(outputText) | color(Color::GrayLight) | hcenter}) |
                     hcenter}) |
         border;
}
} // namespace TTOT::Core::Scenes