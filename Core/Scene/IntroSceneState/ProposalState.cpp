...
namespace TTOT::Core::Scenes::IntroSceneState
{
    ...
    void ProposalState::HandleInput(IntroScene& scene, const ftxui::Event& event)
    {
        ...
        {
            scene.context.eventBus.Publish(Events::InjectPurposeEvent{scene.introContext.adventurePurpose, scene.introContext.guidance});
    ...
    Element ProposalState::Render(IntroScene& scene)
    {
        return vbox({
                    ...
                    separator(),
                    paragraph(scene.introContext.guidance) | size(WIDTH, LESS_THAN, 80) | color(Color::GrayLight),
                    separator(),
