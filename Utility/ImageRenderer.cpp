...
Element ImageRenderer::ImageRender(const std::string &file_path) {
  ...
  std::string resolvedPath = GetAssetPath(file_path).string();
  unsigned char *img_data =
      stbi_load(resolvedPath.c_str(), &width, &height, &channels, 3);
  if (!img_data)
    return text("❌ 이미지를 로드할 수 없습니다: " + file_path) |
  ...
  for (int y = 0; y < new_height; y += 2) {
    ...
    for (int x = 0; x < new_width; ++x) {
      ...
      Color top_color = Color::RGB(img_data[top_idx], img_data[top_idx + 1],
                                   img_data[top_idx + 2]);

      Color bottom_color = Color::RGB(0, 0, 0);
      if (y + 1 < new_height && orig_y_bot < height) {
        int bot_idx = (orig_y_bot * width + orig_x) * 3;
        bottom_color = Color::RGB(img_data[bot_idx], img_data[bot_idx + 1],
                                  img_data[bot_idx + 2]);
      }
