...
private:
  ...
  int _stackedCount = 1;    // 중첩 횟수, 기본 값은 1로 하여 계산 편의
  int _maxStackedCount = 1; // 최대 중첩 횟수, 기본 값은 1로 하여 계산 편의

public:
  StatModifier(int val, uint32_t sourceId,
               StatModifierType calcType = StatModifierType::Flat,
               int stackedCount = 1, int maxStackedCount = 1);
